...
export const generateDemoData = () => {
  ...
  const tickets: Ticket[] = [
    { id: 'TKT-001', ticketNumber: 'TKT-2026-0001', subject: 'EMP Device Not Charging', description: 'EMP device shows 0% charge and will not power up.', requesterId: 'CON-ROLAND', accountId: 'ACC-HAMMER', status: 'Open', priority: 'Urgent', assigneeId: 'USR-LINK', slaDeadline: futureDate(1), messages: [{ sender: 'Roland', senderId: 'CON-ROLAND', text: 'Our EMP device shows 0% charge and wont power up. Sentinels incoming!', time: pastDate(1), isMe: false, isBot: false }], internalNotes: [], createdAt: pastDate(1), updatedAt: timestamp, createdBy: 'CON-ROLAND' },
    { id: 'TKT-002', ticketNumber: 'TKT-2026-0002', subject: 'Operator Console Flickering', description: 'Main operator console display keeps flickering during Matrix operations.', requesterId: 'CON-SPARKS', accountId: 'ACC-LOGOS', status: 'In Progress', priority: 'High', assigneeId: 'USR-LINK', slaDeadline: futureDate(3), messages: [{ sender: 'Sparks', senderId: 'CON-SPARKS', text: 'Main operator console display keeps flickering during Matrix operations.', time: pastDate(3), isMe: false, isBot: false }, { sender: 'Link', senderId: 'USR-LINK', text: 'Running diagnostics now. Can you describe when it started?', time: pastDate(2), isMe: true, isBot: false }], internalNotes: [{ sender: 'Link', senderId: 'USR-LINK', text: 'Likely power coupling issue - ordering replacement part.', time: pastDate(1), isMe: true, isBot: false }], createdAt: pastDate(3), updatedAt: timestamp, createdBy: 'CON-SPARKS' },
    { id: 'TKT-003', ticketNumber: 'TKT-2026-0003', subject: 'Training Upload Failed', description: 'Kung Fu upload stuck at 47%. System shows memory buffer error.', requesterId: 'CON-BINARY', accountId: 'ACC-VIGILANT', status: 'Open', priority: 'Medium', assigneeId: 'USR-NEO', slaDeadline: futureDate(5), messages: [{ sender: 'Binary', senderId: 'CON-BINARY', text: 'Kung Fu upload stuck at 47% for crew member. System shows memory buffer error.', time: pastDate(2), isMe: false, isBot: false }], internalNotes: [], createdAt: pastDate(2), updatedAt: timestamp, createdBy: 'CON-BINARY' },
    { id: 'TKT-004', ticketNumber: 'TKT-2026-0004', subject: 'Request: Additional Hardlines', description: 'Need 3 additional hardline phone locations mapped for downtown extraction zone.', requesterId: 'CON-MORPHEUS', accountId: 'ACC-NEBUCHADNEZZAR', status: 'Resolved', priority: 'Low', assigneeId: 'USR-TRINITY', slaDeadline: pastDate(5), messages: [{ sender: 'Morpheus', senderId: 'CON-MORPHEUS', text: 'We need 3 additional hardline phone locations mapped for downtown extraction zone.', time: pastDate(10), isMe: false, isBot: false }, { sender: 'Trinity', senderId: 'USR-TRINITY', text: 'Mapped and uploaded to your navigation system. Stay safe.', time: pastDate(8), isMe: true, isBot: false }], internalNotes: [], createdAt: pastDate(10), updatedAt: timestamp, createdBy: 'CON-MORPHEUS' },
    { id: 'TKT-005', ticketNumber: 'TKT-2026-0005', subject: 'Sentinel Detection False Positives', description: 'Early warning system triggered 5 false alarms in 24 hours.', requesterId: 'CON-GHOST', accountId: 'ACC-LOGOS', status: 'Open', priority: 'High', assigneeId: 'USR-NIOBE', slaDeadline: futureDate(2), messages: [{ sender: 'Ghost', senderId: 'CON-GHOST', text: 'Early warning system triggered 5 false alarms in the past 24 hours. Crew is exhausted from emergency drills.', time: pastDate(0), isMe: false, isBot: false }], internalNotes: [], createdAt: pastDate(0), updatedAt: timestamp, createdBy: 'CON-GHOST' },
  ];

  // 13. COMMUNICATIONS
  const communications: Communication[] = [
    { id: 'COM-001', type: 'Call', subject: 'Prophecy Discussion', content: 'Discussed latest prophecy interpretations with Morpheus. He believes Neo is ready.', direction: 'Outbound', relatedToType: 'contacts', relatedToId: 'CON-MORPHEUS', outcome: 'answered', duration: 2700, createdAt: pastDate(5), updatedAt: timestamp, createdBy: 'USR-NEO' },
    ...
    { id: 'COM-003', type: 'Call', subject: 'Overdue Payment Discussion', content: 'Called Roland about the outstanding invoice. He mentioned supply chain issues delaying Zion credits.', direction: 'Outbound', relatedToType: 'contacts', relatedToId: 'CON-ROLAND', outcome: 'answered', duration: 900, createdAt: pastDate(2), updatedAt: timestamp, createdBy: 'USR-TRINITY' },
